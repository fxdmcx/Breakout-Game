...
unsigned int solidblockTexture;
unsigned int spriteTexture;
unsigned int paddleTexture;
unsigned int particleTexture;
unsigned int powerup_speed;
unsigned int powerup_sticky;
unsigned int powerup_increase;
unsigned int powerup_passthrough;
unsigned int powerup_confuse;
unsigned int powerup_chaos;
...
void Game::Init()
{
	...
	Shader post_shader("shaders/post_processing.vs", "shaders/post_processing.fs");
	// Configure shaders
	...
	particle_shader.use();
	particle_shader.setInt("particle", 0);
	particle_shader.setMat4("projection", projection);
	// Load textures
	backgroundTexture = loadTexture("textures/background.jpg");
	spriteTexture = loadTexture("textures/awesomeface.png");
	blockTexture = loadTexture("textures/block.png");
	solidblockTexture = loadTexture("textures/block_solid.png");
	paddleTexture = loadTexture("textures/paddle.png");
	particleTexture = loadTexture("textures/paddle.png");

	powerup_speed = loadTexture("textures/powerup_speed.png");
	powerup_sticky = loadTexture("textures/powerup_sticky.png");
	powerup_increase = loadTexture("textures/powerup_increase.png");
	powerup_passthrough = loadTexture("textures/powerup_passthrough.png");
	powerup_confuse = loadTexture("textures/powerup_confuse.png");
	powerup_chaos = loadTexture("textures/powerup_chaos.png");
	// Set render-specific controls
	Renderer = new SpriteRenderer(shader);
	Particles = new ParticleGenerator(particle_shader, particleTexture, 500);
	Effects = new PostProcessor(post_shader, this->Width, this->Height);
	...
	Ball = new BallObject(ballPos, BALL_RADIUS, INITIAL_BALL_VELOCITY, spriteTexture);
...
void Game::ResizeFb(int width, int height)
{
	//this->Width = width;
	//this->Height = height;
	Shader post_shader("shaders/post_processing.vs", "shaders/post_processing.fs");
	Effects = new PostProcessor(post_shader, width, height);
}
...
void Game::SpawnPowerUps(GameObject &block)
{
	if (ShouldSpawn(75)) // 1 in 75 chance
		this->PowerUps.push_back(PowerUp("speed", glm::vec3(0.5f, 0.5f, 1.0f), 0.0f, block.Position, powerup_speed));
	if (ShouldSpawn(75))
		this->PowerUps.push_back(PowerUp("sticky", glm::vec3(1.0f, 0.5f, 1.0f), 20.0f, block.Position, powerup_sticky));
	if (ShouldSpawn(45))
		this->PowerUps.push_back(PowerUp("pass-through", glm::vec3(0.5f, 1.0f, 0.5f), 10.0f, block.Position, powerup_passthrough));
	if (ShouldSpawn(45))
		this->PowerUps.push_back(PowerUp("pad-size-increase", glm::vec3(1.0f, 0.6f, 0.4), 10.0f, block.Position, powerup_increase));
	if (ShouldSpawn(75)) // Negative powerups should spawn more often
		this->PowerUps.push_back(PowerUp("confuse", glm::vec3(1.0f, 0.3f, 0.3f), 15.0f, block.Position, powerup_confuse));
	if (ShouldSpawn(75))
		this->PowerUps.push_back(PowerUp("chaos", glm::vec3(0.9f, 0.25f, 0.25f), 15.0f, block.Position, powerup_chaos));
}
